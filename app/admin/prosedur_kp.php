...
if (isset($_POST['simpan'])) {
    ...
    if ($judul !== '' && $deskripsi !== '') {
        $stmt = $conn->prepare("INSERT INTO prosedur_kp (judul, deskripsi) VALUES (?, ?)");
        $stmt->bind_param("ss", $judul, $deskripsi);
        $stmt->execute();
        $stmt->close();
    }

    header("Location: prosedur_kp.php");
    exit;
}
...
if (isset($_POST['update'])) {
    ...
    if ($id > 0 && $judul !== '' && $deskripsi !== '') {
        $stmt = $conn->prepare("UPDATE prosedur_kp SET judul = ?, deskripsi = ? WHERE id_kp = ?");
...
if (isset($_GET['hapus'])) {
    ...
    if ($id > 0) {
        $stmt = $conn->prepare("DELETE FROM prosedur_kp WHERE id_kp = ?");
...
if ($cari !== '') {
    ...
    $q = mysqli_query($conn, "SELECT * FROM prosedur_kp ORDER BY id_kp DESC");
}
?>
...
<div class="main-content">
    ...
    <div class="topbar">
        <div class="topbar-left">
            <h2>Prosedur Kerja Praktik (KP)</h2>
            <p>Kelola prosedur KP mahasiswa</p>
        </div>
    ...
    <div class="card">
        <div class="toolbar">
            <form method="GET" class="search-form">
                ...
                <a href="prosedur_kp.php" class="btn btn-yellow">
                            ...
                            <td>
                                <div class="action-group">
                                    <button
                                        ...
                                        type="button"
                                        onclick="editData(JSON.parse(this.dataset.kp))"
                                        data-kp="<?= $jsonData ?>"
                                    >
...
<div class="modal" id="modal">
    <div class="modal-box">
        ...
        <div class="modal-header">
            <h3 id="judulModal">Tambah Prosedur KP</h3>
        </div>

        <form method="post" id="kpForm">
            <input type="hidden" name="id" id="id">

            <div class="form-group">
                <label for="judul">Judul Prosedur</label>
                <input type="text" name="judul" id="judul" placeholder="Masukkan judul prosedur" required>
            </div>

            <div class="form-group">
                <label for="deskripsi">Deskripsi</label>
                <textarea name="deskripsi" id="deskripsi" placeholder="Isi prosedur lengkap..." required></textarea>
            </div>
...
function openModal() {
    ...
    judulModal.innerText = "Tambah Prosedur KP";
}
