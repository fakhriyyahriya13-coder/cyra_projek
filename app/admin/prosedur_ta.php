...
if (isset($_POST['simpan'])) {
    ...
    if ($judul !== '' && $deskripsi !== '') {
        $stmt = $conn->prepare("INSERT INTO prosedur_ta (judul, deskripsi) VALUES (?, ?)");
        $stmt->bind_param("ss", $judul, $deskripsi);
        $stmt->execute();
        $stmt->close();
    }

    header("Location: prosedur_ta.php");
    exit;
}
...
if (isset($_POST['update'])) {
    ...
    if ($id > 0 && $judul !== '' && $deskripsi !== '') {
        $stmt = $conn->prepare("UPDATE prosedur_ta SET judul = ?, deskripsi = ? WHERE id_ta = ?");
...
if (isset($_GET['hapus'])) {
    ...
    if ($id > 0) {
        $stmt = $conn->prepare("DELETE FROM prosedur_ta WHERE id_ta = ?");
...
if ($cari !== '') {
    ...
    $q = mysqli_query($conn, "SELECT * FROM prosedur_ta ORDER BY id_ta DESC");
}
?>
...
<div class="main-content">
    ...
    <div class="topbar">
        <div class="topbar-left">
            <h2>Prosedur Tugas Akhir (TA)</h2>
            <p>Kelola prosedur TA mahasiswa</p>
        </div>
    ...
    <div class="card">
        <div class="toolbar">
            <form method="GET" class="search-form">
                ...
                <a href="prosedur_ta.php" class="btn btn-yellow">
                            ...
                            <td>
                                <div class="action-group">
                                    <button
                                        ...
                                        type="button"
                                        onclick="editData(JSON.parse(this.dataset.ta))"
                                        data-ta="<?= $jsonData ?>"
                                    >
...
<div class="modal" id="modal">
    <div class="modal-box">
        ...
        <div class="modal-header">
            <h3 id="judulModal">Tambah Prosedur TA</h3>
        </div>
...
function openModal() {
    ...
    judulModal.innerText = "Tambah Prosedur TA";
}
